...
def save_render_frame():
    ...
    Body: ``{name, index, view?, png_b64, pose}``. Writes to
    ``<scenes-root>/<scene>/renders/<name>/frame_NNNN.png`` (or
    ``frame_NNNN__<view>.png`` for non-``"forward"`` views) and appends
    to ``manifest.json``. Dedupe key is ``(index, view)``; legacy entries
    without ``view`` are normalized to ``forward`` in-place on every write.
    """
    data = request.get_json(force=True)
